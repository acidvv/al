...
hint "Displaying information";
	...
	switch (true) do 
	{
		case (_victim == _killer):
		{
			...
			_msg = format["Ktoś umarł: %1 has killed %1 (MOżliwe że samobójstwo lub się rozłaczyl)",_killer getVariable["realname",name _killer]];
[0,_msg] remoteExec ["life_fnc_broadcast",0];
		...
		case (vehicle _killer isKindOf "Car"):
		{
			...
			_msg = format["Ktoś zginął w wypadku samochodowym: %1 has obliterated %2 with a %3",_killer getVariable["realname",name _killer]];
[0,_msg] remoteExec ["life_fnc_broadcast",0];
		};
		case (vehicle _killer isKindOf "Air"):
		{
			...
			_msg = format["Ktoś zgniał w wypadku lotniczym: %1 has obliterated %2 with a %3",_killer getVariable["realname",name _killer], _victim getVariable["realname",name _victim], _vehName];
[0,_msg] remoteExec ["life_fnc_broadcast",0];
		};
		default
		{
			...
			_msg = format ["I poległ śmiercią bohatera: %1 has killed %2 with Weapon %3 from Distance %4 Meters",_killer getVariable["realname",name _killer], _victim getVariable["realname",name _victim], _killerWep, _distance];
[0,_msg] remoteExec ["life_fnc_broadcast",0];
		};
	};
